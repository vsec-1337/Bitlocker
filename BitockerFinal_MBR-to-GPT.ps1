...
else
{
    ...
    #20 seconds break to let mbr2gpt.exe end as another background process
    Start-Sleep -s 20
}
...
If(!$BLVS)
{
    ...
    #Add TPM protection to the disk.
    ...
    #We wait 10 seconds for disk encryption as another background process
    Start-Sleep -s 10

    ###########PART 3 : Sending recuperation key(s) to AzureAD ###########
    $BLVS = Get-BitLockerVolume | Where-Object {$_.KeyProtector | Where-Object {$_.KeyProtectorType -eq 'RecoveryPassword'}}
    foreach($BLV in $BLVS)
    {
...
{
###########PART 3 : Keys recuperation and export to AzureAD if Bitlocker was already active ###########
$BLVS = Get-BitLockerVolume | Where-Object {$_.KeyProtector | Where-Object {$_.KeyProtectorType -eq 'RecoveryPassword'}} 
    foreach($BLV in $BLVS)
    {
        $Key = $BLV | Select-Object -ExpandProperty KeyProtector | Where-Object {$_.KeyProtectorType -eq 'RecoveryPassword'}
...
########### PART 5 : Key file creation to get it with Kaseya ###########
